...
use crate::raytrace::{
    camera::Camera, material::Lambertian, object::Object, shape::Sphere,
...
fn camera_perspective() {
    ...
    assert_eq!(camera.projection_screen_u, Vector3::new(0., 0., 1.));
    // Vertical projection plane (1 unit in front of the camera, pointing up)
    assert_eq!(camera.projection_screen_v, Vector3::new(0., 9. / 16., 0.));
}
...
fn ray_physics() {
    ...
    let intersection = sphere.hit(&ray).expect("The ray must intersect the sphere");
    ...
    for _ in 0..10_000 {
        sphere
            .material
            .scatter(&mut ray, intersection, sphere.shape.normal(intersection));
